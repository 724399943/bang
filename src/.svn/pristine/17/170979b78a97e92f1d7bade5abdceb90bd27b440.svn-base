      ...
      <div class="main pd">
		  ...
		  <form class="login_form">
            <div class="login_box">
              ...
              <a href="javascript:;" class="login_btn" @click="resetPsw">重设密码</a>
            </div>
          </form>     
  ...
  methods: {
    resetPsw(){
      ...
      if ( !this.dataJson.account ) {
        ...
        this.$http.post('/Shop/Login/forgetPassword', this.dataJson,{emulateJSON:true}).then(function(response){ 
    ...
    getCode : function(){      
      if ( !this.dataJson.account ) {
        ...
        var that = this;
        if ( that.noget == 1 ) {
        that.$http.post('/Shop/Login/getVerifyCode', {account:this.dataJson.account,type:1}, {emulateJSON:true}).then(function(response){
          var returnData = response['data'];
          if( returnData['status'] == '200000' ){
