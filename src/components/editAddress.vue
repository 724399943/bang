<template>
  <div class="content" id="content">
      ...
      <span class="text" @touchstart="editConsignee">保存</span>
		</header>
		<div class="main plg">
          ...
          <ul>              
            ...
            <li class="ute">
                ...
                <input id="JAreavalue" type="hidden" v-model="dataJson.area">
...
export default {
  ...
  methods: {
    getConsignee(){
      this.$http.post('/Shop/Consignee/getConsignee', {id:this.$route.params.id},{emulateJSON:true}).then(function(response){ 
        var returnData = response['data'];
        if( returnData.status == 200000 ){
          this.dataJson = returnData.data.list;
          if( this.dataJson.is_default == 1 ){
    ...
    editConsignee(){
      ...
      var consigneeJson = {id:this.dataJson.id,consignee:this.dataJson.consignee,province:this.dataJson.province,city:this.dataJson.city,county:this.dataJson.county,address:this.dataJson.address,telephone:this.dataJson.telephone,is_default:this.dataJson.is_default}      
      this.$http.post('/Shop/Consignee/editConsignee', consigneeJson,{emulateJSON:true}).then(function(response){ 
        var returnData = response['data'];
        if( returnData.status == 200000 ){
          this.$router.go(-1);
        }else{
    ...
    apifun(){
      ...
      document.getElementById('JLAreaclick').setAttribute('value', that.dataJson['province_name'] + ' - ' + that.dataJson['city_name'] + ' - ' + that.dataJson['county_name']);
      var area = (that.dataJson['area'].split(','));
